...
// 常用操作
  ...
  function oneclickImport(client, subLink) {
    ...
    var sublink = {
      ...
      quantumultx: "quantumultx://configuration?server=" + btoa(subLink).replace(/=/g, '') + "&filter=YUhSMGNITTZMeTl0ZVM1dmMyOWxZMjh1ZUhsNkwzSjFiR1Z6TDNGMVlXNTBkVzExYkhRdVkyOXVaZw",
      shadowrocket: "shadowrocket://add/sub://" + btoa(subLink) + "?remarks=" + appName,
      surge2: "surge2:///install-config?url=" + encodeURIComponent(subLink),
      ...
      success: function (data) {
        ...
        if (data.errcode == 0) {
          ...
          if (user_location == 'code') {
            ...
            $('#fcpay-modal').modal('show');
            $('#to-fcpay').attr("href", data.code);
          }
        }
      }
    });
